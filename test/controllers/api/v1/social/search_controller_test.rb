...
class Api::V1::Social::SearchControllerTest < ActionDispatch::IntegrationTest
  test "user can search videos and creators" do
    ...
    SocialPost.create!(creator_user_id: creator.matrix_user_id, media_upload_id: "upl_search", playback_url: "https://cdn.example.test/video.mp4", caption: "Canvas tote drop")

    get api_v1_social_search_url(q: "canvas"), headers: tep_headers(user, "social:read"), as: :json

    assert_response :success
    assert_equal [ "Canvas tote drop" ], response.parsed_body.fetch("posts").map { |post| post.fetch("caption") }
