...
class PrivacyIntegrationTest < ActionDispatch::IntegrationTest
  ...
  # ============================================================================
  # COMMERCE CHAT — order gating
  # ============================================================================

  test "commerce chat requires order ownership" do
    merchant = CommerceMerchant.create!(
      owner_user_id: @seller.matrix_user_id,
      miniapp_id: "miniapp.chat.test",
      display_name: "Chat Shop",
      status: "active"
    )
    order = merchant.commerce_orders.create!(
      buyer_user_id: @buyer.matrix_user_id,
      payment_id: "pay_chat_test",
      status: "paid",
      subtotal_cents: 1000, total_cents: 1000, currency: "NGN"
    )

    # Viewer (not buyer or seller) tries to create message room
    post api_v1_commerce_order_messages_url(order.order_id),
         headers: tep_headers(@viewer, "commerce:orders")

    assert_response :forbidden
  end

  test "commerce chat requires paid order" do
    merchant = CommerceMerchant.create!(
      owner_user_id: @seller.matrix_user_id,
      miniapp_id: "miniapp.chat.pending",
      display_name: "Pending Shop",
      status: "active"
    )
    order = merchant.commerce_orders.create!(
      buyer_user_id: @buyer.matrix_user_id,
      payment_id: "pay_pending_test",
      status: "pending_payment",
      subtotal_cents: 1000, total_cents: 1000, currency: "NGN"
    )

    # Buyer tries to message on unpaid order
    post api_v1_commerce_order_messages_url(order.order_id),
         headers: tep_headers(@buyer, "commerce:orders")
  ...
  test "commerce chat creates relay room for buyer" do
    # Stub Matrix API for bot room creation
    CommerceRelayService.define_singleton_method(:make_matrix_request) do |*|
      { "room_id" => "!test_relay_room:#{ENV.fetch('MATRIX_DOMAIN', 'tween.im')}" }
    end

    merchant = CommerceMerchant.create!(
      owner_user_id: @seller.matrix_user_id,
      miniapp_id: "miniapp.chat.create",
      display_name: "Create Shop",
      status: "active"
    )
    order = merchant.commerce_orders.create!(
      buyer_user_id: @buyer.matrix_user_id,
      payment_id: "pay_create_test",
      status: "paid",
      subtotal_cents: 1000, total_cents: 1000, currency: "NGN"
    )
    ...
    assert_response :created
    body = response.parsed_body
    assert_not_nil body["room_id"], "relay room should be created"
    assert body["room_id"].start_with?("!"), "room_id should start with ! (Matrix room ID)"
  end
